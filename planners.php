...
<?php
	...
	if (!empty($instlist)){
		...
		foreach($instlist as $inst) {
			...
			$instpeopleqry = "SELECT users.name AS name, users.username AS username, CASE WHEN userfields.type = 'PRIM' THEN fields.name ELSE userfields.fcode END AS field FROM users, userinstitutions, userfields, fields WHERE (userinstitutions.instid = '$instid' AND users.id_user = userinstitutions.uid AND userfields.type = 'PRIM' AND userfields.uid = users.id_user AND fields.code = userfields.fcode) OR (userinstitutions.instid = '$instid' AND users.id_user = userinstitutions.uid AND userfields.type = 'OTHER' AND userfields.uid = users.id_user) LIMIT 10";
